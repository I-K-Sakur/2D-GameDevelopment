...
function Level:update(dt)
    ...
    if love.keyboard.isDown('space') and not self.hit then
        self.hit=true
        --if love.keyboard.isDown("space") and self.hit==false then
                    --self.hit=true
                   for i=2,3 do
                    local posx,posy=self.launchMarker.aliens[1].body:getPosition()
                    local velx,vely=self.launchMarker.aliens[1].body:getLinearVelocity()
                    
                        table.insert(self.launchMarker.aliens,Alien(self.world, 'round', posx, posy, 'Player'))
                        if i==2 then
                            velx=velx-vely/8+60
                            vely=vely-velx/8
                        else
                        ...
                        self.launchMarker.aliens[i].body:setLinearVelocity(velx,vely)
...
--          alienlaunch={}
--         -- if we fired our alien to the left or it's almost done rolling, respawn
--         if xPos < 0 or (math.abs(xVel) + math.abs(yVel) < 1.5) or love.mouse.isDown('1') then
--         self.launchMarker.alien.body:destroy()
--         self.launchMarker = AlienLaunchMarker(self.world)
--              table.insert(alienlaunch,true)
--          else
--             table.insert(alienlaunch,false) 
...
--         --     --end
--         -- end
--          local respawalien=true
--          local i=0
--         for k,alienlaunch in pairs(alienlaunch) do
--             if respawalien then
--                 respawalien=alienlaunch
--             end
--         end
--         -- if respawalien then
--             for k,alien in pairs (self.launchMarker.aliens) do
