...
function PlayState:update(dt)
    ...
    if self.canInput then
        ...
        elseif love.keyboard.wasPressed('right') then
            ...
            x, y = push:toGame(love.mouse.getPosition()) --spec5
            if x >= VIRTUAL_WIDTH - 272 and y >= 16 and x <= VIRTUAL_WIDTH - 16 and y <= 272 then
                mouseHoverX = math.floor((x - (VIRTUAL_WIDTH - 272)) / 32)
                mouseHoverY = math.floor((y - 16) / 32)
                if self.boardHighlightX ~= mouseHoverX or self.boardHighlightY ~= mouseHoverY then gSounds['select']:play() end
                self.boardHighlightX = mouseHoverX
                self.boardHighlightY = mouseHoverY
                -- print(math.floor((x - (VIRTUAL_WIDTH - 272)) / 32), math.floor((y - 16) / 32))
            end
        ...
        --self.check=self.board:checkTable()
        end
    end 
end
    ...
    if matches then
     --self.timer=self.timer+Match
        ...
        for k, match in pairs(matches) do
            for k,match in pairs(match) do
            
            
            self.score = self.score + variety*4+50 *3
        ...
        if not self.board:checkTable() then
            Mat=true
            self.canInput=true
...
function PlayState:render()
    ...
    self.board:render()
 if Mat==true then
    Timer.tween(3,{[self]={transitionAlpha=0}})
